...
The analysis of the Mean Squared Error (MSE) results highlights that the Random Forest model performs best, achieving the lowest MSE of 294.9206, which represents a significant improvement (~17%) over the baseline model's 352.9058.
The baseline, which assumes constant predictions (mean), serves as a reference point for comparison. Linear Regression and Decision Tree models performed similarly with MSE values of 338.9219 and 335.1666, respectively, indicating that they struggled to capture non-linear relationships in the data.
The K-Nearest Neighbors (KNN) model, with an MSE of 336.4332, also failed to outperform the simpler models, likely due to the absence of localized patterns or the influence of feature scaling and neighbor selection. These results confirm that Random Forest effectively captures complex, non-linear relationships between features and song popularity, outperforming other approaches. 

The Decision Tree and K-Nearest Neighbors (KNN) models underperformed compared to Random Forest, and I suspect this has to do with some fo the inherent limitations of the respective models when applied to this dataset.
For the Decision Tree, I think that it overfitting is the most likely problem when faced with complex relationships, especially when the depth is limited.
I noticed that the Decision Tree *really* focused on is_edm, which caused it to miss subtle patterns that could have improved its generalization. This aligns with my expectation that a single tree would not fully capture the non-linear nature of song popularity.
For the KNN model, I feel that it failed to outperform due to the high dimensionality. Outside of this, picking 50 neighbours could have potentially smoother the model out too much and missed some finer details.

These two models, I felt were unlikely to perform as well, but I was surprised at exactly how poorly they performed even compared to a simple regression. However, we did gleam some interesting insights across the board. 
